export function initAdminPage(appState) {
  ...
  function updateReservationsList() {
    ...
    container.innerHTML = appState.filteredReservations
      .map(
        (reservation, index) => `
        <div class="p-8 hover:bg-amber-50 transition-all duration-300 slide-up" style="animation-delay: ${
          index * 0.05
        }s;">
          <div class="flex justify-between items-start">
            <div class="flex-1">
              <div class="flex items-center space-x-6 mb-4">
                ...
                <span class="px-4 py-2 rounded-full text-sm font-bold ${getStatusClass(
                  reservation.status
                )}">
              ...
              <div class="grid md:grid-cols-2 gap-4 text-amber-700 mb-4">
                ...
                <div class="flex items-center">
                  ...
                  <span class="text-lg">${formatDate(reservation.date)} à ${
          reservation.time
        }</span>
                </div>
                <div class="flex items-center">
                  <i class="fas fa-users mr-3 text-amber-500"></i>
                  <span class="text-lg">${reservation.guests} personne${
  ...
  function updateStatistics() {
    ...
    const confirmed = appState.reservations.filter(
      (r) => r.status === "confirmed"
    ).length;
    const pending = appState.reservations.filter(
      (r) => r.status === "pending"
    ).length;
    const cancelled = appState.reservations.filter(
  ...
  function applyFilters() {
    ...
    appState.filteredReservations = appState.reservations.filter(
      (reservation) => {
  ...
  async function updateReservationStatus(backendId, newStatus) {
    ...
    const reservation = appState.reservations.find(
      (r) => r.__backendId === backendId
    );
    if (!reservation) {
      return;
    }
    ...
    try {
      ...
      if (res.ok && json.isOk) {
        window.showToast(
          `Réservation ${getStatusText(newStatus).toLowerCase()}`,
          "success"
        );
        // update local state
        const idx = appState.reservations.findIndex(
          (r) => r.__backendId === backendId
        );
        if (idx !== -1) {
          appState.reservations[idx] = {
            ...appState.reservations[idx],
            status: newStatus,
          };
          appState.filteredReservations = [...appState.reservations];
  ...
  // --- MENU CRUD ---
  let editingItemId = null; // Track which item we're editing
  ...
  async function loadMenuItems() {
    ...
    try {
      // Fetch full menu items with _id by making authenticated request
      const res = await fetch("/api/menu", {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (!res.ok) throw new Error("Erreur chargement menu");

      // Fetch all MenuItem documents to get _id
      const menuItemsRes = await fetch("/api/menu/all", {
        headers: { Authorization: `Bearer ${token}` },
      });

      let allItems = [];
      if (menuItemsRes.ok) {
        allItems = await menuItemsRes.json();
      } else {
        // Fallback: use the grouped menu data
      ...
      if (allItems.length === 0) {
        container.innerHTML =
          '<tr><td colspan="5" class="text-center text-amber-600 py-8">Aucun plat</td></tr>';
      } else {
        const rows = allItems
          .map((item, idx) => {
            ...
            const escapedPrice = (item.price || "").replace(/'/g, "\\'");
            ...
            <tr>
              ...
              <td class="px-4 py-2 space-x-2">
                <button class="btn-secondary px-3 py-1 rounded" onclick="editMenuItem('${itemId}','${escapedName}','${escapedCategory}','${escapedDescription}','${escapedPrice}')"><i class="fas fa-edit"></i></button>
                <button class="btn-danger px-3 py-1 rounded" onclick="deleteMenuItemPrompt('${itemId}','${escapedName}')"><i class="fas fa-trash"></i></button>
              </td>
            </tr>
          `;
          })
      ...
      container.innerHTML =
        '<tr><td colspan="5" class="text-center text-amber-600 py-8">Erreur chargement menu</td></tr>';
    }
  }
  ...
  async function addOrUpdateMenuItem(event) {
    ...
    try {
      ...
      if (editingItemId) {
        ...
        if (!res.ok) throw new Error("Erreur modification plat");
        window.showToast("Plat modifié avec succès", "success");
        editingItemId = null;
        ...
        window.showToast("Plat ajouté avec succès", "success");
      }
  ...
  function updateFormButton(isEditing) {
    const submitBtn = document.querySelector(
      '#add-menu-form button[type="submit"]'
    );
    const cancelBtn = document.getElementById("cancel-edit-btn");

    if (submitBtn) {
      if (isEditing) {
        submitBtn.innerHTML = '<i class="fas fa-save mr-2"></i>Modifier';
  ...
  window.editMenuItem = function (id, name, category, description, price) {
    editingItemId = id;
    ...
    window.showToast("Modifiez les champs puis cliquez sur Modifier", "info");
  ...
  window.deleteMenuItemPrompt = async function (itemId, itemName) {
    ...
    try {
      const delRes = await fetch(`/api/menu/${itemId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      });

      if (delRes.ok) {
        window.showToast("Plat supprimé avec succès", "success");
        loadMenuItems();
  ...
  document
    .getElementById("add-menu-form")
    ?.addEventListener("submit", addOrUpdateMenuItem);
  loadMenuItems();
  ...
  function renderAdminOrders() {
    ...
    const totalAmount = adminOrders.reduce(
    ...
    container.innerHTML = adminOrders
      .map(
        (order, index) => `
      <div class="bg-white border-2 border-amber-200 rounded-2xl overflow-hidden hover:shadow-lg transition-all slide-up" style="animation-delay: ${
        index * 0.05
      }s;">
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 p-6 text-white">
          <div class="flex justify-between items-start">
            <div>
              <p class="text-sm opacity-90">Commande #${
                order._id
                  ? order._id.substring(order._id.length - 8).toUpperCase()
                  : "UNKNOWN"
              }</p>
              <p class="text-lg font-semibold mt-1">${new Date(
                order.orderDate
            ...
            <div class="text-right">
              ...
              <p class="text-sm opacity-90 mt-1">${order.items.length} article${
        ...
        <div class="p-6">
          ...
          <div class="grid md:grid-cols-2 gap-4 mb-6 pb-6 border-b border-amber-100">
            ...
            <div class="text-right">
              ...
              <span class="inline-block px-3 py-1 rounded-full text-sm font-bold ${getOrderStatusClass(
                order.status
              )}">
          ...
          <div class="bg-amber-50 rounded-lg p-4 mb-6">
            <div class="flex justify-between items-center">
              ...
              <span class="text-2xl font-bold text-amber-600">${order.totalPrice.toFixed(
                2
              )}€</span>
            </div>
          </div>
